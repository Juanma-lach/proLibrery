...
//    });
    ...
    Path.map("#/cliente/new").to(function () {
        $('#indexContenidoJsp').spinner();
        //var paramsObject = param().defaultizeUrlObjectParameters(param().getUrlObjectFromUrlString(this.params['url']));
        oClienteControl.new($('#indexContenido'), null, oClienteModel, oClienteView);
        $('#indexContenidoJsp').empty();
        return false;
    });
    Path.map("#/cliente/new/:url").to(function () {
        $('#indexContenidoJsp').spinner();
        var paramsObject = param().defaultizeUrlObjectParameters(param().getUrlObjectFromUrlString(this.params['url']));
        oClienteControl.new($('#indexContenido'), paramsObject, oClienteModel, oClienteView);
        $('#indexContenidoJsp').empty();
        return false;
    });
